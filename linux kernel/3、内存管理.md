...
// MMU
    MMU内存控制单元，管理内存并把虚拟地址转换成物理地址的硬件
...
1、段选择符和段寄存器
    (1) 段选择符
        ...
        > 结构
            ...
            请求特权级别(Requested Privilege Level)  2 ~ 0 (2位), 提供了段保护信息
            只有请求者特权级RPL高于(数字低于)或等于相应的描述符特权级DPL，描述符才能被存取
    (2) 段寄存器
        为了方便快速找到段选择符，处理器提供段寄存器，专门放段选择符
        > 6个段寄存器
            cs、ss、ds、es、fs 和gs
...
4、地址转换(逻辑地址->线性地址)
    先检查段选择符中的T1字段，确定GDT还是LDT(gdtr/ldtr)
...
```
// 线性到物理
    '分页单元'将线性地址转换成物理地址，其中一个关键的任务是把所有请求的访问类型与线性地址的访问权限相比较，如果这次内存访问无效，则产生缺页异常
...
// 页框
    分页单元把所有的RAM分成固定长度的页框(page frame)，一个页框包含一个页
    页框是物理页
...
// 基本思路是通过页目录和页表两极映射实现从线性地址到物理地址的转换
...
// 权限控制
    ...
    页和页表一般有2个权限，由目录项或页表项标志User/Supervisor控制的。若此标志为0，只有内核态(CPL=0)才能对页寻址。若为1，都能寻址
...
(3) 体系结构与分区
    区的实际使用与体系结构密切相关
(4) x86-64的分区
    ZONE_DMA在x86-32上包含的页都在0-16MB的内存范围内
    ZONE_HIGHMEM在x86-32上为高于896MB的所有物理内存(在其他体系上，由于所有内存都被直接映射，所以ZONE_HIGHMEM为空)
    ZONE_NOMAL独享前两取剩余的空间，在x86-32上为16-896M的所有物理内存。

// 理论上x86_64系统CPU的寻址空间是16EB，但是实际上只有虚拟地址的最低48位才会在地址转换时被使用
    因为
    1) 现在还用不到完整的64位寻址空间，所以硬件也没必要支持那么多位的地址
    2) 地址宽度越大，操作系统做虚拟地址转换时越累
```
