...
在并行程序中保护共享数据用到'同步'，同步分为'阻塞同步'和'非阻塞同步'，阻塞同步会造成死锁、活锁、性能低下等问题，为了降低风险和提高效率，现在大多使用'非阻塞同步'

1、非阻塞同步(Non-blocking Synchronization)的几种方案
    分为 无干扰(obstruction-free)、无锁(lock-free)、无等待(wait-free)
    (1) obstruction-free
