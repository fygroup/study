...
const char *p1 ="hello";    // 指向字符常量的指针，p1本身不是常量，可以p1++
char *const p2 = "world";   // 指针常量，可以指向非常量的字符，但是"world"存储在只读存储区，所以 p2[1] = 'a' 错误
...
// 这些函数可以代替旧的函数，它们的优点 避免不必要的'临时对象'的产生
...
(2) weak_ptr
    ...
    // lock()会把weak_ptr提升为一个shared_ptr对象，引用数+1
...
std::static_pointer_cast()      // 当指针是智能指针时候，向上转换，类似static_cast
...
int a[5] = {1,2,3,4,5};
*(*(&a + 1) - 1)
// &arr 代表整个数组，它的步长是整个数组所占的字节数， &arr 的类型是 int(*)[5]
// &arr+1 指向数组 arr 最后一个元素的下一个地址， *(&arr+1)-1 则是指向 arr 最后的元素，所以 *(*(&arr+1)-1) 的值是 5
```
