...
4. 然后真实服务器做成请求响应，其余信息不变之后根据自己的路由信息将这个响应数据包发送回给客户，并且源IP地址还是VIP。

1、客户端发送数据 CIP -> VIP{VIPMAC}
2、负载均衡器(VIP:VIPMAC)收到数据包后，其余信息不变，然后据各个真实服务器的负载情况，将请求报文的数据帧的"目标MAC地址"改为"真实服务器的MAC地址" CIP -> VIP{RIPMAC}
3、将修改的数据帧在服务器组的局域网上发送
4、真实服务器(RIP:RIPMAC)收到请求的数据包，发现自己有一个"隐藏"的 VIP，于是接收这个数据包，并交由对应的上层应用处理
5、处理完成后，将响应数据包直接返回给客户端 VIP -> CIP

(1) DR模式是通过MAC地址改写机制实现转发，所以负载均衡与真实服务器要在一个局域网
(2) 真实服务器要将VIP绑定到lo设备上，因为:
	1) 真实服务器收到数据包，解包后，发现


负载均衡器修改请求数据包的 { 目标MAC 地址 } 为 真实服务器的 MAC 地址，其余信息不变。负载均衡器和真实服务器必须在同一局域网内，否则负载均衡器无法替换请求数据包的 { 目标MAC 地址 } 为 真实服务器的 MAC 地址
真实服务器收到请求的数据包，发现自己有一个 “隐藏“ 的 VIP，于是接收这个数据包，并交由对应的上层应用处理
处理完成后，将响应数据包直接返回给客户端。此时在真实服务器上查看 TCP 连接为：VIP ➡️ CIP
