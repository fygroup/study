...
2、TCP重传机制
     ...
     (2) 快速重传
          ...
          注意到在重传后，发送方继续正常的数据传输。TCP不需要等待对方确认刚才的重传
          例如：
...
3、TCP的拥塞处理
     (1) 流量控制
          为了避免在源主机和目的主机之间的某处网络上发生拥塞而导致的分组丢失，必须控制发送端每次发送的分组数
          分组丢失的指示：发生超时或受到重复的ACK
     (2) 为什么要控制流量
...
1、重传定时器
     ...
     可能发生两种情况：若在计时器超时之前收到对报文段的确认，则撤销计时器；若在收到对特定报文段的确认之前计时器超时，则重传该报文，并把计时器复位
2、坚持计时器
     对付零窗口通知而设立的(上述的Zero Window)

3、保活计时器(keeplive timer)
     可检测到一个空闲连接的另一端何时崩溃或重启。
