...
(3) 阻塞和未决
    当一个信号产生时，内核通常在进程表中以某种形式设置一个标志。当对信号采取这种动作时，我们说向进程递送了一个信号，而且对该信号的动作是系统默认动作或捕捉该信号，


    信号产生 ----------> 信号处理
    信号产生 ----------> 阻塞信号处理
              信号未决
    1) 进程可以选择阻塞（Block）某个信号。被阻塞的信号产生时将保持在未决状态，直到进程解除对此信号的阻塞，才执行递达的动作。注意，阻塞和忽略是不同，只要信号被阻塞就不会递达，而忽略是在递达之后可选的一种处理动作。
    2) 信号的相关状态字
        每一位代表一个信号
    ...
    3) 过程
        信号 ---> 进程
        当对应信号的block为1时，该信号被阻塞，那么相应的pending位为1
        当对应信号的block为0时，该信号解除阻塞，那么相应的pending位为0
    4) 屏蔽状态字可以读写，未决状态字只能读；这是信号设计机制
...
(3) API
    ...
    3) 经过sigaction安装的信号都能传递信息给信号处理函数，而经过signal安装的信号不能向信号处理函数传递信息。对于信号发送函数来说也是一样的。
